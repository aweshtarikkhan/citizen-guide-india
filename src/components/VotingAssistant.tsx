...
const VotingAssistant = () => {
  ...
  const sendMessage = async (text: string) => {
    ...
    // Test mode: append ?testGemini=1 to the URL to force direct Gemini API
    const forceGemini = typeof window !== "undefined" && new URLSearchParams(window.location.search).get("testGemini") === "1";
    ...
    try {
      const resp = await fetch(CHAT_URL, {
        ...
        body: JSON.stringify({ messages: allMsgs, ...(forceGemini ? { provider: "gemini" } : {}) }),
      });
          ...
          <div className="px-4 py-3 bg-foreground text-background flex items-center gap-3 shrink-0">
            ...
            <div className="flex-1">
              <p className="font-display font-semibold text-sm">
                Matdaan Assistant
                {typeof window !== "undefined" && new URLSearchParams(window.location.search).get("testGemini") === "1" && (
                  <span className="ml-2 px-1.5 py-0.5 text-[9px] rounded bg-blue-400 text-black font-bold">GEMINI TEST</span>
                )}
              </p>
